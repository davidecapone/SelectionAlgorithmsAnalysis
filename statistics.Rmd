...
quickselect_worst_stats %>% ggplot( aes(x = size) ) + 
  geom_ribbon(aes(ymin = quickselect_average-quickselect_variance/2, 
                  ymax = quickselect_average+quickselect_variance/2), fill = "grey70") +
  geom_line(aes(y=quickselect_average, colour="quickSelect"),size=1 )+
  stat_smooth(aes(y = quickselect_average), method = "lm", formula = y ~ x + I(x^2), size = 1, se=F, color="red") +
  labs(x = "Dimensione", y = "Tempo", 
...
static_size_stats  %>% ggplot( aes(x = k) ) + 
  ...
  geom_smooth(aes(y=heapselect_average,colour="heapSelect"),size=1) +
  geom_smooth(aes(y=medianmediansselect_average,colour="medianMediansSelect"),size=1) +
  labs(x = "k", y = "Tempo", 
       title = TeX("Analisi asintotica k incrementale e dimensione fissata"),
       subtitle = "40 campioni per k",
       caption = "Scala lineare") +
  scale_color_manual(name = "Algoritmi di selezione", values = c("quickSelect" = quickselect_color, "heapSelect" = heapSelect_color, "medianMediansSelect" = medianMediansSelect_color))
```
...
```{r}
# sospettiamo una correlazione tra parametro k e heapselect, verifichiamo:
cor(static_size_stats$k, static_size_stats$heapselect_average, method = "pearson")
# valore prossimo ad 1: correlazione lineare positiva molto forte

# dimostriamo invece la scarsa correlazione per gli altri algoritmi:
cor(static_size_stats$k, static_size_stats$quickselect_average, method = "pearson")
cor(static_size_stats$k, static_size_stats$medianmediansselect_average, method = "pearson")

```
```{r}
# valutiamo quindi i tempi di esecuzione utilizzando una max heap per dimostrare che questi decrescono all'aumentare di k:
```
